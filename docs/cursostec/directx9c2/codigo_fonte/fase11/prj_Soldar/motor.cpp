...
void inicializarTexto( void ) 
{
  ...
  g_hr = D3DXCreateFont( g_device, 
	  nAltura, nLargura, nPeso, 
	  nMipmap, bItalico, 
...
void gerarModeloSimplificado( void )
{
	...
	D3DXMESH_MANAGED, 
	g_objeto3d->GetFVF() | D3DFVF_NORMAL, 
	g_device,
	&g_meshSimplificado);
	...
	if ( FAILED ( g_hr ) )
	{
		MessageBoxA (0, "CloneMeshFVF() falhou.", 
			"gerarModeloSimplificado()", MB_OK);
	} // endif


	DWORD wy_flags = D3DXWELDEPSILONS_WELDPARTIALMATCHES | D3DXWELDEPSILONS_WELDALL;

	D3DXWELDEPSILONS *wy_tolerancias  = NULL;

	// Simplificação do mesh
	g_hr = D3DXWeldVertices ( g_meshSimplificado,
		wy_flags,
		wy_tolerancias, &adjInfo[0], &adjInfo[0], 
		NULL, NULL );

	// Verifica se houve erro
	if ( FAILED ( g_hr ) )
		MessageBoxA (0, "D3DXWeldVertices() falhou.", 
		"gerarModeloSimplificado()", MB_OK);
