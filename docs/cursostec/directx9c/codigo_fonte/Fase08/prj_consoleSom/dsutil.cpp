//-----------------------------------------------------------------------------
// File: DSUtil.cpp
// Contém classes de um framework de trabalho com o DirectSound
// que fazem a leitura e gravação de arquivos wav bem como tocá-los
// através de um buffer do DirectSound. Utilize esta classe como
// ponto de partida para adicionar novas funcionalidades.
...
// Acesso aos utilitários definidos pelas macros
#include "motor.h" 

//-----------------------------------------------------------------------------
// contrutor CSoundManager::CSoundManager()
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Destrutor CSoundManager::~CSoundManager()
//-----------------------------------------------------------------------------
CSoundManager::~CSoundManager()
{
    SAFE_RELEASE( m_pDS ); 
...
// Inicializa o objeto IDirectSound e também configura o formato do
// buffer primário. Esta função deve ser chamada antes de qualquer
// outra.
//-----------------------------------------------------------------------------
                                   ...
                                   DWORD dwCoopLevel )
{
    ...
    // Cria o objeto IDirectSound usando o dispositivo de som primário
    if( FAILED( hr = DirectSoundCreate8( NULL, &m_pDS, NULL ) ) )
        return DXTRACE_ERR( TEXT("DirectSoundCreate8"), hr );

    // Configura o nível de cooperação do DirectSound como prioridade
...
//  Configura o formato do buffer primário para o formato especificado 

// Aviso: configurar o formato do buffer primário e então usar o mesmo
// objeto dsound para o DirectMusic bagunça o DirectMusic!

//  Exemplo, configurando o formato do buffer primário como estéreo 22kHz, 16-bit
//  dwPrimaryChannels = 2
//  dwPrimaryFreq     = 22050, 
//  dwPrimaryBitRate  = 16
//-----------------------------------------------------------------------------
...
// Retorna a interface do ouvinte 3D associada com o buffer primário.
//-----------------------------------------------------------------------------
HRESULT CSoundManager::Get3DListenerInterface( LPDIRECTSOUND3DLISTENER* ppDSListener )
{
    ...
    *ppDSListener = NULL;
		...
		// Obtém o buffer primário requisitando controle 3d
                                                  ...
                                                  (VOID**)ppDSListener ) ) )
    {
		...
		// Libera o buffer primário desde que ele não é necessário
    SAFE_RELEASE( pDSBPrimary );
