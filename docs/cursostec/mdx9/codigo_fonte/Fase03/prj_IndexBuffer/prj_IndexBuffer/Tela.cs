// Prj_IndexBuffer - Arquivo: Tela.cs
// Esse projeto ilustra como utilizar o IndexBuffer
...
namespace prj_IndexBuffer
{
  public partial class Tela : Form
  {
    ...
    public void montar_triangulos()
    {
      ...
      // o buffer de índices
      triangulo = new CustomVertex.PositionColored[4];

      // Os vértices estão alinhados na mesma posição de profundidade
      float zpos = 1.0f;

      // p0      
      triangulo[0].SetPosition ( new Vector3 (1.0f, 1.0f, zpos));
      triangulo[0].Color = vermelho;

      // p1
      triangulo[1].SetPosition(new Vector3 (-1.0f, 1.0f, zpos));
      triangulo[1].Color = verde;

      // p2
      triangulo[2].SetPosition ( new Vector3(-1.0f, -1.0f, zpos));
      triangulo[2].Color = azul;

      // p3
      triangulo[3].SetPosition (new Vector3 (1.0f, -1.0f, zpos));
      triangulo[3].Color = Color.LightBlue.ToArgb();
