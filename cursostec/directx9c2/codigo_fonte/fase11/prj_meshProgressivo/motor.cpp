...
ID3DXPMesh *g_meshProgressivo = NULL;
...
{	
	...
	if ( g_meshProgressivo != NULL) g_meshProgressivo->Release();
...
void inicializarTexto( void ) 
{
  ...
  g_hr = D3DXCreateFont( g_device, nAltura, nLargura, 
	  nPeso, nMipmap, bItalico, 
    nCharset, nPrecisao, nQualidade,
	nFamilia, sFonte, &gdxFonte );  

  if(FAILED (g_hr) ) 
    MessageBox(NULL, "Texto: falha na inicialização", 
	"inicializarTexto()", MB_OK);   
...
void renderizar_Geometria()
{
	...
	{
		g_meshProgressivo->SetNumVertices ( 
			g_meshProgressivo->GetNumVertices() + g_nDetalhe );
	...
	if ( g_meshProgressivo != NULL )
	desenharObjeto( (ID3DXMesh *)g_meshProgressivo, &g_props);
	
	// Mostra informações dos modelos 3d
	sprintf (info, "mdl normal: %d", g_objeto3d->GetNumVertices() );	
	mostrarTexto(20, 20, info );  

	sprintf (info, "mdl progressivo: %d", 
		g_meshProgressivo->GetNumVertices() );		
	mostrarTexto(420, 20, info);

} // renderizar_Geometria();


// [--- $gerarModeloProgressivo() - Simplificação do modelo 3d#
void gerarModeloProgressivo( void )
{
	...
	g_hr = D3DXGeneratePMesh ( g_meshLimpo, &adjInfo[0], NULL, NULL, 1,
		D3DXMESHSIMP_VERTEX , &g_meshProgressivo  );

		// Verifica se houve erro
	if ( FAILED ( g_hr ) )
		MessageBoxA (0, "D3DXGeneratePMesh() falhou.", 
		"gerarModeloProgressivo()", MB_OK);
